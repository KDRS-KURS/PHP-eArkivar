...
//					$strThisElement = $xml->name;
				...
				case $xml::CDATA:	// 4 CDATA node
					print 'CDATA line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::ENTITY_REF:	// 5 Enity Reference node
					print 'ENTITY_REF line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::ENTITY:	// 6 Entity Declaration node
					print 'ENTITY line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::PI:	// 7 Processing Instruction node
					print 'PI line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::COMMENT:	// 8 Comment node
					print 'COMMENT line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::DOC:	// 9 Document node
					print 'DOC line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::DOC_TYPE:	// 10 Document Type node
					print 'DOC_TYPE line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::DOC_FRAGMENT:	// 11 Document Fragment node
					print 'DOC_FRAGMENT line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::NOTATION:	// 12 Notation node
					print 'NOTATION line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::WHITESPACE:	// 13 Whitespace node
					print 'WHITESPACE line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::SIGNIFICANT_WHITESPACE:	// 14 Significant Whitespace node
					print 'SIGNIFICANT_WHITESPACE line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::END_ENTITY:	// 16 End Entity
					print 'END_ENTITY line starts here: ' . $xml->name . PHP_EOL;
				break;
				
				case $xml::XML_DECLARATION:	// 17 XML Declaration node
					print 'XML_DECLARATION line starts here: ' . $xml->name . PHP_EOL;
				break;
			...
			if ('' !== $xml->name) {
				print 'name = ' . $xml->name . ', nodeType = ' . $xml->nodeType. PHP_EOL;
			}
			
			if ('' !== $xml->localName) {
				print 'localName = ' . $xml->localName . ', nodeType = ' . $xml->nodeType. PHP_EOL;
			}
...
//						$atttribNodeType = $xml->nodeType;	// const int DEFAULTATTRS = 2 ;
			...
			if ($xml->hasValue) {
				print 'hasValue' . ', nodeType = ' . $xml->nodeType. ', value = [' . $xml->value . ']' . PHP_EOL;
			}
			
			if ($xml->isDefault) {
				print 'isDefault' . ', nodeType = ' . $xml->nodeType. PHP_EOL;
			}
			
			if ($xml->isEmptyElement) {
				print 'isEmptyElement' . ', nodeType = ' . $xml->nodeType. PHP_EOL;
			}
			
			
			if ('' !== $xml->depth) {
				print 'depth = ' . $xml->depth . ', nodeType = ' . $xml->nodeType. PHP_EOL;
			}
			
			if ('' !== $xml->prefix) {
				print 'prefix = ' . $xml->prefix . ', nodeType = ' . $xml->nodeType. PHP_EOL;
			}
			
			if ('' !== $xml->xmlLang) {
				print 'xmlLang = ' . $xml->xmlLang . ', nodeType = ' . $xml->nodeType. PHP_EOL;
			}
